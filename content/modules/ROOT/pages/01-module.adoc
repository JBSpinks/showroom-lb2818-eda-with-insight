...
// .. Login Details:
. *Ansible Automation Platform URL:* {aap_controller_web_url}[window=_blank]
...
. *AAP2.5 - OCP4 Custom Decision* - is a custom decision environment built from the default environment. This decision environment includes a custom Event-Driven Ansible plugin, k8s, and is hosted on quay.io/mitsharm/eda/de-for-ocp-aap-25:latest.
****
